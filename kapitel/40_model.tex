...
\comment{Nachfolgenden Abschnitt verkürzen? Auslassen von selbsterklärenden Werten?}
...
\begin{table}[h!]
	\centering
	\begin{tabular}{ | >{\centering\arraybackslash}m{0.16\textwidth} | m{0.33\textwidth} | m{0.41\textwidth} | } 
		\hline
		...
		\centertable{Damit ein eigener Lebenszyklus möglich ist, können sie unabhängig von anderen Objekten leben.}  \\ 
		\hline
		Zugriffsmethode & 
		\centertable{Auf Daten und Funktionen wird mithilfe einer Entität zugegriffen.} &
		\centertable{Können als Aggregate Root oder durch dieses direkten Zugriff erfahren.} \\ 
		\hline
	\end{tabular}
