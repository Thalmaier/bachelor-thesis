...
\subsubsection{Untergliederung des Baskets in kleinere Bestandteile}

\comment{Obige Überschrift lassen oder entfernen?}
...
Zwischen dem Persistieren des eingefrorenen Warenkorb und der Initiierung des PaymentProcesses kann aufgrund von parallel ablaufenden Anfragen der Warenkorb weiterhin bearbeitet werden. Diese \Gls{Race Condition} kann Bedingungen, welche zuvor explizit überprüft und erfüllt waren, nun invalide gestalten. Beispielsweise kann in einem anderen Thread ein externer API-Aufruf die einzige Zahlungsmethode zwischen Zeile 4 und 7 entfernen, wodurch die Initiierung eigentlich Fehlschlagen sollte, jedoch trotzdem Ausgeführt wird. Es entsteht ein invalider Zustand des Datensatzes, weshalb weitere Operationen auf den Warenkorb fehlschlagen und zusätzlich rechtliche Probleme entstehen könnten. Verdeutlicht wird dieser Effekt im Sequenzdiagramm \ref{fig:VarC-Sequence-Alt}. Beide Anfragen richten sich an den gleichen Warenkorb. Der rot markierte Bereich stellt die Zeitspanne dar, in welcher andere Prozesse den Warenkorb weiterhin bearbeiten können, obwohl dies nicht vorgesehen ist. Da die Baskets vor den Modifikationen der anderen Threads geladen werden, ist das Abfangen eines solchen Fehlerzustandes ebenfalls nicht realisierbar.

\begin{figure}[htbp]
	\centering
	\includesvg[inkscapelatex=false, width=0.80\textwidth]{svg/VarC-Sequence Alt.svg}
	\caption{Vereinfachtes Sequenzdiagramm zur Initiierung des Bezahlvorgangs in Variante C}
	\label{fig:VarC-Sequence-Alt}
\end{figure}

Als Folge dessen entstehen weitere Herausforderungen für das Domainmodell. Die möglichen Verschränkungen von Prozessen führt zu  .



 

Verdeutlicht wird dieser Effekt bei der Benutzung eines Message Brokers, falls exemplarisch die einzelnen Prozesse in verschiedenen Microservices ablaufen. Hierbei wird die konkrete Initiierung in einem anderen Service durchgeführt, welcher bei Empfang eines bestimmten Events ausgeführt wird. Folglich feuert die Funktion nach Einfrieren des Warenkorbs ein solches Event ab. Ein möglicher Programmablauf ist in Diagramm \ref{fig:VarC-Sequence-Alt} zu sehen. 

Der rot markierte Bereich stellt die Zeitspanne dar, in welcher andere Prozesse den Warenkorb weiterhin bearbeiten können und dadurch die eigentliche Initiierung invalide, jedoch weiterhin durchgeführt wird.   Clients, welche den aktuellen Stand des Bestellprozess abfragen, erhalten bis zur Speicherung des Payment Aggregates einen veralteten Stand. Dies stellt einen bedeutenden Bruch der Invarianten dar. Folglich kann das Zusammenspiel zwischen Basket und PaymentProcess nicht durch eventuelle Konsistenz gelöst werden ohne ein hohes Risiko einzugehen.
...
\comment{Innerhalb eines solchen Domain-Modells ist es möglich, dass die Kalkulation des Warenkorbs erst bei expliziter Abfrage der Ergebnisse erfolgt, wodurch Berechnungszeit eingespart werden kann. Ein solches Vorgehen erlaubt es bei einer hohen Auslastung der Systeme multiplikativ an Performance zu gewinnen. Zusätzlich müssen weniger Nutzlast zwischen den Clients und den Server gesendet werden, da die einzelnen Aggregates eine geringere Anzahl an Attributen beinhalten. Es muss allerdings genauer Untersucht werden, ob die zusätzlichen Datenbankabfragen diesen Effekt negieren oder dennoch insgesamt ein positiver Einfluss auf die Applikationsgeschwindigkeit zu verzeichnen ist. }
...
Damit eine ähnlichere Situation zu Produktivanwendungen geboten werden kann, wurde bei Cloud-Anbietern jeweils eine MongoDB und PostgreSQL in einem frankfurter Datenzentrum mit 8 Gigabyte Arbeitsspeicher und 4 virtuellen Kernen angemietet. Der vorherige Testaufbau wird unter Verwendung dieser Datenspeichern wiederholt und tabellarisch dem Anhand unter \ref{fig:performance-database} hinzugefügt. Das Diagramm \ref{fig:PerformanceDatabase} bietet die dazugehörige grafische Auswertung.
